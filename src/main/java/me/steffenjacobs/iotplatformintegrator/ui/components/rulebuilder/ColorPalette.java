package me.steffenjacobs.iotplatformintegrator.ui.components.rulebuilder;

import java.awt.Color;

/** @author Steffen Jacobs */
public class ColorPalette {

	static Color MAIN_COLOR = new Color(Integer.parseInt("96ceb4", 16));
	static Color SECOND_COLOR = new Color(Integer.parseInt("ffeead", 16));
	static Color THIRD_COLOR = new Color(Integer.parseInt("ff6f69", 16));
	static Color SECOND_SHADE_COLOR = new Color(Integer.parseInt("ffcc5c", 16));
	static Color MAIN_SHADE_COLOR = new Color(Integer.parseInt("88d8b0", 16));
	static Color TRANSPARENT = new Color(1, 1, 1, 0);

}
